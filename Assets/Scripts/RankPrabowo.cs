...
public class RankPrabowo : MonoBehaviour {
    ...
    public void GetData() {
        ...
        using (var command = connection.CreateCommand()) {
            command.CommandText = "SELECT * FROM  \"data_provinsi\" WHERE prabowo > jokowi ORDER BY \"prabowo\" DESC;";
            using IDataReader reader = command.ExecuteReader();
            while (reader.Read()) {
                //add gameobject
                ranking.Add(reader["nama_provinsi"].ToString(), float.Parse(reader["prabowo"].ToString()));
